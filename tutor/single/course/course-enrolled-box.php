        ...
        if ( $wp_query->query['post_type'] !== 'lesson') {
            ...
            if ( $lesson_url ) { 
                ...
                <a href="<?php echo esc_url($lesson_url); ?>" class="<?php echo esc_attr($button_class); ?>" data-course_id="<?php echo get_the_ID(); ?>">
                    <?php
                        if($retake_course) {
                            _e( 'Retake This Course', 'visapass' );
                        } else if( $completed_percent <= 0 ){
                            _e( 'Start Course', 'visapass' );
                        } else {
                            _e( 'Continue Course', 'visapass' );
                        }
                    ?>
                </a>
                <?php 
    ...
    <div class="tutor-single-course-segment  tutor-course-enrolled-wrap">
        <p>
            ...
            <?php
                ...
                echo sprintf(__('You have been enrolled on %s.', 'visapass'),  "<span>". date_i18n(get_option('date_format'), strtotime($enrolled->post_date)
                    )."</span>"  );
                ?>
        </p>
